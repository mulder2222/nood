...
@php
    ...
    $__isCustomer = auth()->guard('customer')->check();
    $__isWishlist = false;
    if ($__isCustomer) {
        $__customer = auth()->guard('customer')->user();
        $__isWishlist = (bool) ($__customer?->wishlist_items
...
<x-shop::layouts>
    ...
    @if ((core()->getConfigData('general.general.breadcrumbs.shop')))
        <!-- Breadcrumbs (desktop) aligned with main gallery container -->
        <div class="container px-[60px] max-1180:px-0 max-lg:hidden">
            <div class="flex ml-[132px]">
                <x-shop::breadcrumbs
                    name="product"
                    :entity="$product"
                />
    ...
    <v-product data-is-customer="{{ $__isCustomer ? '1' : '0' }}" data-is-wishlist="{{ $__isWishlist ? '1' : '0' }}">
        ...
        <div class="max-1180:hidden">
            ...
            >
                ...
                <x-shop::tabs.item
                    id="tech-tab"
                    class="container mt-[60px] !p-0"
                    :title="trans('smarthome::app.products.view.technic')"
                    :is-selected="false"
                >
                    <div class="container mt-[60px] max-1180:px-5">
                        <p class="text-lg text-zinc-500 max-1180:text-sm">
    ...
    <div class="container mt-6 grid gap-3 !p-0 max-1180:px-5 1180:hidden">
        ...
        >
            <x-slot:header class="bg-gray-100 max-md:!py-3 max-sm:!py-2">
                <p class="text-base font-medium 1180:hidden">
                    @lang('smarthome::app.products.view.technic')
                </p>
            </x-slot>
    ...
    @pushOnce('scripts')
                            ...
                            <div class="relative max-w-[590px] max-1180:w-full max-1180:max-w-full max-1180:px-5 max-sm:px-4">
                                ...
                                <p class="mt-[22px] flex items-center gap-2.5 text-2xl !font-medium max-sm:mt-2 max-sm:gap-x-2.5 max-sm:gap-y-0 max-sm:text-lg">
                                    {!! $product->getTypeInstance()->getPriceHtml() !!}
                                                                     @php
                                    $__discountPercent = $product->getTypeInstance()->getDiscountPercent();
                                    @endphp

                                    @if ($__discountPercent > 0)
                                        <span class="inline-block ml-auto bg-sentraRed px-2.5 py-0.5 text-sm font-semibold text-white">
                                            -{{ $__discountPercent }}%
                                        </span>
                                    @endif
                                </p>
                                ...
                                @endif
                                <ul class="list-none mt-6 grid gap-3 max-w-max">
                                    <li class="flex items-center gap-2">
                                        <img
                                            src="{{ bagisto_asset('images/green-check.svg') }}"
                                            class="h-[25px] w-[25px] rounded-full"
                                            alt="Sterk punt"
                                        />
                                        <span class="text-green-500 font-bold text-sm">Gratis thuisbezorgd vanaf €50</span>
                                    </li>
                                    <li class="flex items-center gap-2">
                                        <img
                                            src="{{ bagisto_asset('images/green-check.svg') }}"
                                            ...
                                            alt="Sterk punt"
                                        />
                                        <span class="text-green-500 font-bold text-sm">Binnen 3 dagen bij uw thuis</span>
                                    </li>
                                </ul>
                                ...
                                <div class="mt-8 flex max-w-[470px] gap-4 max-sm:mt-4">
                                    ...
                                    @if (core()->getConfigData('sales.checkout.shopping_cart.cart_page'))
                                        ...
                                        <x-shop::button
                                            type="submit"
                                            class="secondary-button border-none text-white w-full max-w-full max-md:py-3 max-sm:rounded-lg max-sm:py-1.5 bg-green-500"
                                ...
                                <p class="mt-6 text-lg text-zinc-500 max-sm:mt-1.5 max-sm:text-sm">
                                                                    @php
                                    try {
                                        $__prosCons = app('BaWe\\ProsCons\\Repositories\\ProsConRepository')->getByProduct($product->id);
                                    } catch (\Throwable $e) {
                                        $__prosCons = ['pros' => [], 'cons' => []];
                                    }
                                    $__pros = collect($__prosCons['pros'] ?? []);
                                    $__cons = collect($__prosCons['cons'] ?? []);
                                @endphp

                                @if ($__pros->count() || $__cons->count())
                                    <div class="mt-6 grid gap-3 max-w-[590px]">
                                        <h3 class="text-black font-semibold">Pluspunten & Minpunten</h3>
                                        @if ($__pros->count())
                                            <ul class="list-none grid gap-2">
                                                @foreach ($__pros as $__pro)
                                                    @php $__proText = is_array($__pro) ? ($__pro['text'] ?? '') : ($__pro->text ?? ''); @endphp
                                                    @if (trim($__proText) !== '')
                                                            <li class="flex items-start gap-2">
                                                                <svg class="mt-0.5 h-[20px] w-[20px] shrink-0 min-w-[20px] min-h-[20px]" viewBox="0 0 20 20" aria-hidden="true" focusable="false">
                                                                    <circle cx="10" cy="10" r="10" fill="#22c55e"></circle>
                                                                    <rect x="9" y="5" width="2" height="10" fill="white"></rect>
                                                                    <rect x="5" y="9" width="10" height="2" fill="white"></rect>
                                                                </svg>
                                                                <span class="text-black text-sm">{{ $__proText }}</span>
                                                            </li>
                                                    @endif
                                                @endforeach
                                            </ul>
                                        ...
                                        @if ($__cons->count())
                                            <ul class="list-none grid gap-2">
                                                @foreach ($__cons as $__con)
                                                    @php $__conText = is_array($__con) ? ($__con['text'] ?? '') : ($__con->text ?? ''); @endphp
                                                    @if (trim($__conText) !== '')
                                                            <li class="flex items-start gap-2">
                                                                <svg class="mt-0.5 h-[20px] w-[20px] shrink-0 min-w-[20px] min-h-[20px]" viewBox="0 0 20 20" aria-hidden="true" focusable="false">
                                                                    <circle cx="10" cy="10" r="10" fill="#ef4444"></circle>
                                                                    <rect x="5" y="9" width="10" height="2" fill="white"></rect>
                                                                </svg>
                                                                <span class="text-grey-500 text-sm">{{ $__conText }}</span>
                                                            </li>
                                                    @endif
                                                @endforeach
                                            </ul>
        ...
        <script type="module">
            app.component('v-product', {
                ...
                data() {
                    const el = document.querySelector('v-product');
                    const ds = el ? el.dataset : {};

                    return {
                        isWishlist: ds.isWishlist === '1',
                        isCustomer: ds.isCustomer === '1',
                        is_buy_now: 0,
                        isStoring: {
                            addToCart: false,
                            buyNow: false,
